...
export class OfficeSettingsService {
  ...
  async getSettings() {
    const settings = await this.prisma.officeSetting.findFirst({
      orderBy: {
        createdAt: 'asc',
      },
    });
  ...
  //Patch
  async updateSettings(dto: UpdateOfficeSettingDto) {
    const settings = await this.prisma.officeSetting.findFirst({
      orderBy: {
        createdAt: 'asc',
      },
    ...
    const workStartTime = dto.workStartTime ?? settings.workStartTime;

    const workEndTime = dto.workEndTime ?? settings.workEndTime;

    if (this.timeToMinutes(workStartTime) >= this.timeToMinutes(workEndTime)) {
    ...
    const latitude = dto.officeLatitude ?? settings.officeLatitude;

    const longitude = dto.officeLongitude ?? settings.officeLongitude;
    ...
    const updatedSettings = await this.prisma.officeSetting.update({
      where: {
        id: settings.id,
      },
      ...
      data: {
        ...
        ...(dto.workingDays !== undefined && {
          workingDays: [...new Set(dto.workingDays)],
        }),
