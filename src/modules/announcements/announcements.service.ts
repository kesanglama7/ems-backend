...
export class AnnouncementsService {
  ...
  private validate(
    ...
    if (!title?.trim() || !body?.trim())
      throw new BadRequestException('Title and body must contain text.');
    ...
    if (audience === 'DEPARTMENT' && !departmentId)
      throw new BadRequestException(
        'departmentId is required for department announcements.',
      );
    if (audience === 'ALL_EMPLOYEES' && dto.departmentId)
      throw new BadRequestException(
        'departmentId is only allowed for department announcements.',
      );
  ...
  async update(id: string, dto: UpdateAnnouncementDto) {
    ...
    if (!['DRAFT', 'SCHEDULED'].includes(existing.status))
      throw new ConflictException(
        'Published or archived announcements cannot be edited.',
      );
    ...
    if (!result.count)
      throw new ConflictException('Announcement changed while editing.');
  ...
  async schedule(id: string, dto: ScheduleAnnouncementDto) {
    ...
    if (existing.status !== 'DRAFT')
      throw new ConflictException('Only drafts may be scheduled.');
    ...
    if (!result.count)
      throw new ConflictException('Announcement is no longer a draft.');
    return this.getAdmin(id);
  }
  ...
  async publish(id: string) {
    ...
    if (announcement.status !== 'DRAFT' && announcement.status !== 'SCHEDULED')
      throw new ConflictException(
        'Announcement has already been published or archived.',
      );
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (!changed.count) {
          ...
          if (!noExpiry.count)
            throw new ConflictException(
              'Announcement is no longer publishable.',
            );
        }
        const notice = await tx.announcement.findUniqueOrThrow({
          where: { id },
        });
        ...
        return {
          success: true,
          data: { ...notice, recipientCount: userIds.length },
        };
      },
      { timeout: 60_000 },
    );
      ...
      for (const row of due) {
        try {
          ...
          if (!(error instanceof ConflictException))
            this.logger.error(
              `Scheduled announcement ${row.id} failed.`,
  ...
  async archive(id: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!result.count)
        throw new ConflictException(
          'Announcement is already archived or does not exist.',
        );
  ...
  async listAdmin(query: AnnouncementQueryDto) {
    const where: Prisma.AnnouncementWhereInput = query.status
      ? { status: query.status }
      : {};
  ...
  async listMine(userId: string, query: AnnouncementQueryDto) {
    ...
    return {
      success: true,
      data: items.map((item) => this.withMyReceipt(item)),
      pagination: { page: query.page, limit: query.limit, total },
    };
  }
  ...
  async loginPending(userId: string) {
          ...
          this.activeWhere(userId),
          {
            ...
            OR: [
              { receipts: { some: { userId, readAt: null } } },
              {
                ...
                receipts: { some: { userId, acknowledgedAt: null } },
              },
            ],
          },
        ],
    ...
    return {
      success: true,
      data: data.map((item) => this.withMyReceipt(item)),
    };
  }
  ...
  private withMyReceipt<
    T extends {
      receipts: readonly { readAt: Date | null; acknowledgedAt: Date | null }[];
    },
  >(data: T) {
